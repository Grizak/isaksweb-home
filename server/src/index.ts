...
app.post("/api/auth/login", (req: Request, res: Response) => {
  ...
  if (username === adminUsername && password === adminPassword) {
    ...
    res.status(401).json({ success: false, message: "Invalid credentials" } as AuthResponse);
  }
});
...
app.put("/api/dashboard/projects/:id", requireAuth, (req: Request, res: Response) => {
  ...
  dashboardProjects[projectIndex] = { ...dashboardProjects[projectIndex], ...req.body };
  res.json({ success: true, project: dashboardProjects[projectIndex] });
});

// Add new project (protected)
app.post("/api/dashboard/projects", requireAuth, (req: Request, res: Response) => {
  const newProject: Project = {
...
app.delete("/api/dashboard/projects/:id", requireAuth, (req: Request, res: Response) => {
  const id = parseInt(req.params.id);
  const projectIndex = dashboardProjects.findIndex(p => p.id === id);
...
app.put("/api/dashboard/learning", requireAuth, (req: Request, res: Response) => {
  currentlyLearning = req.body.currentlyLearning;
  res.json({ success: true, currentlyLearning });
});

// Refresh GitHub data (protected)
app.post("/api/dashboard/refresh-github", requireAuth, async (req: Request, res: Response) => {
  try {
    await loadProjects();
    res.json({ success: true, message: "GitHub data refreshed", projects: projectsFromGithub });
  } catch (error) {
    res.status(500).json({ success: false, message: "Failed to refresh GitHub data" });
  }
});
...
loadProjects().then(() => {
  app.listen(PORT, () => {
    ...
    console.log(`Dashboard credentials: ${process.env.ADMIN_USERNAME || 'admin'} / ${process.env.ADMIN_PASSWORD || 'password'}`);
  });
});
